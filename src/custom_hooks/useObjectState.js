...
function useObjectState(initialState = {}) {
  ...
  function setState(stateUpdateObject, callback = () => {}) {
    ...
    const newState = { ...state, ...stateUpdateObject }; // spread the old state and then the incoming state together to create a distinct newState object.  StateUpdateObject's keys/values will overwrite matching keys/values on the old state - this is like Object.assign
